...
Safe, bounded read-only search of a directory tree by metadata (name/pattern, type, depth). No predicate that executes a subprocess or mutates the filesystem is exposed.
